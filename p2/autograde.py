...
def docker_build():
    ...
    subprocess.check_output(["docker", "build", ".", "-t", "p2"], env=environment)
    ...
    try:
        subprocess.check_output(
            [
                ...
                "grpc_tools.protoc",
                "-I=.",
                f"--python_out=compile",
                f"--grpc_python_out=compile",
                "matchdb.proto",
            ],
            stderr=subprocess.DEVNULL,
        )
...
def test_input(case_no, full_point):
    ...
    expected_outputs_ignore_cache = [line.split("*")[0] for line in expected_outputs]
    client_outputs_ignore_cache = [line.split("*")[0] for line in client_outputs]
