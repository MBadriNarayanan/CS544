...
def download_helper():
    ...
    if not os.path.exists(helper):
        ...
        subprocess.run(f"wget {PROJECT_REMOTE_URL}{helper} -O {helper}", shell=True)
...
def tester_main():
    ...
    def ignore(_dir_name, _dir_content):
        return [".git", ".github", "__pycache__", ".gitignore", "*.pyc"]
...
def run_consumer(partition_nums):
    ...
    try:
        ...
        if result.returncode != 0:
            raise Exception(
                "Failed to run consumer script for partitions:", partition_nums
            )
    except subprocess.CalledProcessError as e:
        raise Exception(
            "Failed to run consumer script:" + str(e) + "partitions:", partition_nums
        )
        ...
        for line in file:
            try:
                ...
                if all(
                    ...
                    return "Invalid keys in the output of debug.py. Keys must be: 'station_id','date','degrees','partition'"
            except Exception as e:
    ...
    for _ in range(15):
        ...
        try:
            for i in range(4):
                ...
                partition_offsets[str(i)] = partition_dict["offset"]
            else:
                break
        except Exception as e:
            error_msg = str(e)
        ...
        return f"Failed to generate and read /src/partition-{i}.json from within the container: {error_msg}"
