...
def topics_created(url):
    ...
    for topic in topic_details:
        ...
        if partitions != expected_partitions:
            print(
                f"Topic '{topic_name}' has incorrect partition count: Expected:{expected_partitions}, Found:{partitions}"
            )
            return False
...
def producer_messages(url):
    global Stations
    consumer = KafkaConsumer(bootstrap_servers=url, auto_offset_reset="earliest")
    ...
    for topic, messages in batch.items():
        ...
        for message in messages:
            if str(message.key, "utf-8") not in Stations:
                print(
                    f"Key must be a station name (first letters capitalized), instead got: {message.key}"
                )
                return False
